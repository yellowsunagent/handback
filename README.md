...
Legacy timestamp dates become calendar days in the device’s local time zone at migration. Legacy name-based data is migrated conservatively: the original version 1 storage is retained, and ambiguous ownership or inconsistent records block migration instead of guessing. Preserve the installation when migration fails; do not delete app storage to work around an error.
